...
class FilesContentValidator {
  ...
  def validateFilesContent ( importedData: ( ControlData, List[SaleData], List[UnitData], List[LotData], 
      List[OptionData], List[ConditionData], List[CommentData] ) ) : ImportResult = {
